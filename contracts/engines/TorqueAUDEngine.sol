...
contract TorqueAUDEngine is TorqueEngine {
    ...
    TorqueAUD private immutable i_torqueAUD;
    ...
    constructor(
        ...
        i_torqueAUD = TorqueAUD(torqueAUD);
    }
    ...
    function depositCollateralAndMintTorqueAUD(
        uint256 amountCollateral,
        uint256 amountTorqueAUDToMint,
        uint16 dstChainId,
        ...
        require(amountTorqueAUDToMint > 0, "Mint amount must be greater than 0");
    ...
    function redeemCollateralForTorqueAUD(
        ...
        // CHECKS
        require(amountCollateral > 0, "Amount must be greater than 0");
        require(amountTorqueAUDToBurn > 0, "Burn amount must be greater than 0");
        require(dstChainId > 0, "Invalid destination chain");

        // EFFECTS
        _burnTorque(amountTorqueAUDToBurn, msg.sender);
    ...
    function liquidate(address user, uint256 debtToCover) external moreThanZero(debtToCover) nonReentrant {
        ...
        // Calculate amounts
        uint256 tokenAmountFromDebtCovered = getTokenAmountFromAud(debtToCover);
