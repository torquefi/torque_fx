...
contract TorqueXAGEngine is TorqueEngine {
    ...
    function depositCollateralAndMintTorqueXAG(
        ...
        // INTERACTIONS
        bytes memory message = abi.encode(amountTorqueXAGToMint, msg.sender);
    ...
    function _lzReceive(
        ...
        // Additional custom logic for minting TorqueXAG
        if (_message.length > 0) {
            (uint256 amountTorqueXAGToMint, address user) = abi.decode(_message, (uint256, address));
    ...
    function redeemCollateralForTorqueXAG(
        ...
        // INTERACTIONS
        bytes memory message = abi.encode(amountTorqueXAGToBurn, msg.sender);
        MessagingFee memory fee = _quote(dstChainId, message, adapterParams, false);
        _lzSend(
            dstChainId,
            message,
